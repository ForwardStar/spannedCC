...
class LCTprt{
    ...
    struct Link_Cut_Tree
    {
        ...
        struct node{
            edge e;
            int ts;
            node(int u1=0,int v1=0,int ts1=0,int te1=0){
                e=edge(u1,v1,te1);
                ts=ts1;
            }
        };
        std::vector<node> relation;
        struct PRT{
            int ls,rs,div;
            node c;
            PRT(){
        ...
        PRT* tree;
        int number=0;
        void build(typename std::vector<node>::iterator begin,
                   typename std::vector<node>::iterator end,
                   int fa,
                   bool is_left_child);
        void query(int* parent, int now,int ts,int te);
        ...
        LCTprt() {}
        LCTprt(TemporalGraph * Graph, double t_fraction);
        ~LCTprt() {};
};
