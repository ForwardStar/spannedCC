...
int ctsf::find(int * parent, int u) {
...
bool ctsf::unioN(int * parent, int u, int v) {
...
void ctsf::construct_disjoint_set(int * parent, int ts, int te) {
...
// int ctsf::find_an_index(int t, int ts, int te) {
...
std::stringstream ctsf::solve(int n, int ts, int te) {
...
// void ctsf::update(TemporalGraph * Graph) {
//     int *parent = new int[n];
...
ctsf::ctsf(TemporalGraph * Graph, double t_fraction) {
    ...
    for (int ts = 0; ts <= t1; ++ts) {
        ...
        construct_disjoint_set( parent, ts, t1);
        for (int te = ts; te <= t1; ++te) {
            std::vector<std::pair<int, int>>::iterator it;
            for (it = Graph->temporal_edge[te].begin(); it != Graph->temporal_edge[te].end(); it++) {
                if (unioN(parent, it->first, it->second)) {
                    relation[te].push_back(edge(it->first,it->second,ts));
                    // if (actual_time[ts].size() == 0 || actual_time[ts][actual_time[ts].size() - 1] != te) {
                    //     actual_time[ts].push_back(te);
                    //     relation[ts].push_back(std::vector<std::pair<int, int>>());
                    // }
